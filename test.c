...
void *pthread_service(void* sfd)
{
	...
	while(1)
	{
		...
		switch(header.message_type)
		{
					...
					if(j == 1)//exclusive lock have locked,stop request a lock
						{
							if(header.control_code == 1)//request a lock
								{	
									async_lock_response(&header,0);//result = 1:success,result = 0:failure
									tcp_server_write(connectfd,&header,MESSAGE_HEADER_SIZE,0);
								}
						}
					else
						{
							if(header.control_code == 1)//request a lock
								{	
									async_lock_response(&header,1);//result = 1:success,result = 0:failure
									i++;
									tcp_server_write(connectfd,&header,MESSAGE_HEADER_SIZE,0);
								}
							if (header.control_code == 0)//release a lock
								{//result = 1:exclusive lock was granted,result = 2:shared lock was granted
									int result;
									result = 1;
									async_lock_response(&header,result);
									tcp_server_write(connectfd,&header,MESSAGE_HEADER_SIZE,0);
									if(result == 1)
										{
									...
									if(result == 2)
										...
										j = 2;//shared lock was granted

									if(result == 3)

										j = 3;//exclusive lock was not granted
								}	
						}	
			...
			case AsyncDeviceClear://unlock
				{
					printf("receive AsyncDeviceClear from client\n");
...
int fatal_error(hislip_message *send_message,uint8_t error_code,char *message[])
{
	...
	if(message)
	{
		...
		send_message->data_length = big_end(len);
	}
	else
	send_message->data_length = 0;
	return 0;
...
int non_fatal_error(hislip_message *send_message,uint8_t error_code,char *message[])
{
	memset(send_message,0,sizeof(hislip_message));
	send_message->prologue = htons(MESSAGE_PROLOGUE);
	send_message->message_type = Error;
	send_message->control_code = error_code;
	if(message)
	{
		int len = 0;
...
int async_device_clear_acknowledge(hislip_message *send_message,uint8_t future)
{
	memset(send_message,0,sizeof(hislip_message));
	send_message->prologue = htons(MESSAGE_PROLOGUE);
        send_message->message_type = AsyncDeviceClearAcknowledge;
        send_message->control_code = future;
	return 0;
}
...
int async_remote_local_response(hislip_message *send_message)
{
	memset(send_message,0,sizeof(hislip_message));
	send_message->prologue = htons(MESSAGE_PROLOGUE);
        send_message->message_type = AsyncRemoteLocalResponse;
        send_message->control_code = 0;
	return 0;
}
